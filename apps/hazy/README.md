...
2. **Database (any Postgres, free options available)** — easiest free
   ...
   ```bash
   # from packages/db/ (put the same DATABASE_URL in packages/db/.env.local)
...
5. **Run it** — from the repo root, `bun install` then `bun run dev`
   (`turbo` starts hazy on **:3100** and hazy-note on :3000). Or just this
   app: `bun run dev --filter=hazy`.

## Scripts

Run from the repo root via `turbo` (`bun run <task>`), or in `apps/hazy/`
directly with `bun run <script>`.

| Command | What it does |
|---|---|
| `bun run dev` | Dev server (`--filter=hazy` for just this one) |
| `bun run build` / `bun run start` | Production build / run |
| `bun run lint` | Biome |
| `bun run check-types` | `tsc --noEmit` |
| `bun run cf:build` / `cf:deploy` / `cf:preview` | OpenNext → Cloudflare Workers |

Schema, migrations and the `db:*` scripts live in **`packages/db`** — see
`packages/db/README.md`.

## Why there's a manual SQL trigger

`saved_urls.search_vector` (a Postgres `tsvector`, GIN-indexed) powers
full-text search. Drizzle manages the column and index declared in the
schema, but keeping it *populated* on insert/update is a trigger — that's
not something `drizzle-kit push` expresses, so it lives in
`packages/db/sql/search-vector-trigger.sql` and is applied via
`bun db:trigger` (from `packages/db/`). Re-run it any time you reset the
database.

## Shared database with hazy-note

This is one app in a Turborepo. The **hazy-note** app (`apps/hazy-note`, the
capture → notes → compare → graph → export flow) runs against this same
database and the same Clerk tenant, so its `users`, `saved_urls`, and
`collections` rows *are* these rows — a URL saved here shows up there and vice
versa.

The schema for both apps lives in **`packages/db`** (`@repo/db`). `src/db/`
here is just a thin wrapper: `schema.ts` re-exports `@repo/db/schema`, and
`index.ts`'s `getDb()` gates the shared client with hazy's setup check.
Migrations, the search-vector trigger, and the full runbook are in
`packages/db/README.md`.
